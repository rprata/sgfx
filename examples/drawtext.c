...
int main(int argc, char **argv) {
  if (ibutterfree_init(argc, argv) == IBUTTERFREE_OK) {
    ...
    if (ibutterfree_create_surface(&surface, &desc) == IBUTTERFREE_OK) {
      ...
      ibutterfree_draw_text(&surface, "Hello World", xres / 7, yres / 7,
                            xres / 40, yres / 40, 0xFF0000FF, 0x000000FF);
      ibutterfree_draw_text(&surface, "Hello World", xres / 7, yres / 3,
                            xres / 20, yres / 20, 0x00FF00FF, 0x000000FF);
      ibutterfree_draw_text(&surface, "Hello World", xres / 7, yres / 2,
                            xres / 16, yres / 16, 0x0000FFFF, 0x000000FF);
      ibutterfree_draw_text(&surface, "Hello World", xres / 7, yres / 1.5,
                            xres / 13, yres / 13, 0xFFFFFFFF, 0x000000FF);
