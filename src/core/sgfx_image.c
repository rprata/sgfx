...
unsigned char *__loadBitmapFile(const char *filename,
...
void __flip_vertically(unsigned char *pixels, const size_t width,
                       const size_t height, const size_t bytes_per_pixel) {
...
void __flip_horizontally(unsigned char *pixels, const size_t width,
                         const size_t height, const size_t bytes_per_pixel) {
  int i = 0;
  int j = 0;
  int k = 0;
...
int32_t *__resize_bilinear(int32_t *input, int sourceWidth, int sourceHeight,
                           int targetWidth, int targetHeight) {
  int32_t *temp =
      (int32_t *)malloc(targetWidth * targetHeight * sizeof(int32_t));
...
SGFX_RET sgfx_draw_image(SGFXSurface *surface, const char *filename, int x,
  ...
  if (surface) {
    if (type == SGFX_IMAGE_TYPE_BMP) {
      ...
      if ((width != bitmapInfoHeader.biWidth && width != -1) ||
          (height != bitmapInfoHeader.biHeight && height != -1)) {
        __flip_vertically(bitmapData, bitmapInfoHeader.biWidth,
                          bitmapInfoHeader.biHeight, bpp);
        int32_t *original_image =
            (int32_t *)malloc(bitmapInfoHeader.biWidth *
                              bitmapInfoHeader.biHeight * sizeof(int32_t));
        for (i = 0; i < bpp * bitmapInfoHeader.biHeight; i += bpp) {
          for (j = 0; j < bpp * bitmapInfoHeader.biWidth; j += bpp) {
            ...
            int32_t color =
                ((bitmapData[absPosition] << 24) & 0xFF000000) |
        ...
        int32_t *resized_image =
            __resize_bilinear(original_image, bitmapInfoHeader.biWidth,
                              bitmapInfoHeader.biHeight, width, height);
        ...
        __flip_horizontally(bitmapData, bitmapInfoHeader.biWidth,
                            bitmapInfoHeader.biHeight, bpp);
        for (i = 0; i < bpp * bitmapInfoHeader.biHeight; i += bpp) {
          for (j = 0; j < bpp * bitmapInfoHeader.biWidth; j += bpp) {
            long absPosition = j + bitmapInfoHeader.biWidth * i;
      ...
      if ((width != w && width != -1) || (height != h && height != -1)) {
        ...
        int32_t *resized_image =
            __resize_bilinear(original_image, w, h, width, height);
      ...
      if ((width != w && width != -1) || (height != h && height != -1)) {
        ...
        for (i = 0; i < bpp * h; i += bpp) {
          for (j = 0; j < bpp * w; j += bpp) {
            long absPosition = j + w * i;
            int32_t color =
                ((pngData[absPosition] << 24) & 0xFF000000) +
