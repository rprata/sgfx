...
static int _lua_ibutterfree_init(lua_State *L) {
  if (ibutterfree_init(0, NULL) == IBUTTERFREE_OK) {
    ...
    if (ibutterfree_get_message_error() != NULL)
      lua_pushlstring(L, ibutterfree_get_message_error(),
                      strlen(ibutterfree_get_message_error()));
    return 2;
  }
}
...
static const struct luaL_Reg ibutterfree[] = {{"init", _lua_ibutterfree_init},
