...
IBUTTERFREE_RET __ibutterfree_render(IButterFreeSurface *surface, char *bitmap,
                                     int x, int y, int width, int height,
                                     int32_t background, int32_t foreground) {
  int i, j, color;
...
IBUTTERFREE_RET ibutterfree_draw_text(IButterFreeSurface *surface,
  ...
  for (i = 0; i < strlen(text); i++) {
    if (__ibutterfree_render(surface, ibutterfree_font8x8_basic[(int)text[i]],
                             x + i * (width + SPACE), y, width, height,
                             background, foreground) == IBUTTERFREE_ERROR) {
      return IBUTTERFREE_ERROR;
    }
  }
  return IBUTTERFREE_OK;
